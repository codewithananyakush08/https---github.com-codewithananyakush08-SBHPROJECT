...
def homeownerlogout(request):
    if'homeownerid' in request.session:
        ...
        return redirect('sign')
def homeownerbase(request):
    return render(request,'homeownerbase.html')

def homechangepassword(request):
    if not 'homeownerid' in request.session:
     messages.error(request,"You are not logged in")
     return redirect('sign')  
    homeownerid=request.session.get('homeownerid')
    if request.method == 'POST':
       ...
       try:
          ...
          return redirect('sign')           
    return render(request,'homechangepassword.html',{'homeownerid':homeownerid})
...
def approveapp(request,id):
      ...
      project.status='under_contruction' 
      project.save()
