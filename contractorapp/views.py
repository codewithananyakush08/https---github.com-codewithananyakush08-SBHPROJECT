...
def contractorbase(request):
    return render(request,'contractorbase.html')

def conchgpwd(request):
    if not 'contractorid' in request.session:
     messages.error(request,"You are not logged in")
     return redirect('sign')  
    contractorid=request.session.get('contractorid')
    if request.method == 'POST':
       ...
       try:
          ...
          return redirect('sign')           
    return render(request,'conchgpwd.html',{'contractorid':contractorid})
...
def applyproject(request,id):
      ...
      if request.method=='POST':
         ...
         app.save()
         messages.success(request,"Project application submitted Succcessfully")
         return redirect('contractorviewprojects')
...
def addprogress(request,id):
      ...
      if request.method=='POST':
         ...
         pu=ProgressUpdate(
            project=project,update_text=update_text,image=image,progress_percent=progress_percent,updated_by=contractor
         )
